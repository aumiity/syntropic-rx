...
### POS Page — UX Improvements ✅
- returnFocusToSearch() ใช้ click event delegation (document click) แทนใส่ทุก fn
  - filter: ไม่มี modal เปิด (ยกเว้น toast-container) + active element ไม่ใช่ input/textarea/select
  - ใช้ setTimeout 50ms เพื่อรอ browser event loop
- Search debounce 200ms — ลด 500 error จากพิมพ์เร็ว
- Keyboard nav (↑↓ Enter) ใน search modal ผูกกับ search-input-modal (ไม่ใช่ search-input นอก)
- Search results ORDER BY: trade_name match ก่อน → search_keywords match ทีหลัง
- Discount modal: auto focus+select เมื่อเปิด + Enter=ตกลง, Esc=ยกเลิก
- Pay modal: ช่องส่วนลดท้ายบิล default = sum ของ item discounts ทุกตัวในตะกร้า

### Known Issues
- SESSION 500 error เกิดเป็นครั้งคราวตอนพิมพ์เร็ว — SESSION_DRIVER=file แล้ว ยังไม่หายสนิท
- searchDropdown ไม่มีใน DOM แล้ว — ใช้ dummy object แทน: `const searchDropdown = { classList: { add: () => {}, remove: () => {} } }`
