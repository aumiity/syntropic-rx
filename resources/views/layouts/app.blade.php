...
function generateLabelPreview(label, settings, previewMaxWidthPx = 380, options = {}) {
    ...
    labelContent += `<div style="${buildRowStyle(rowStyles.dosage_line1)}">ทานครั้งละ ${escapeHtml(label.dosage || '')} วันละ ${escapeHtml(label.frequency || '')}</div>`;
    ...
    // Row 7: Indication
    if (label.indication) {
        labelContent += `<div style="${buildRowStyle(rowStyles.indication)}">ข้อบ่งใช้: ${escapeHtml(label.indication)}</div>`;
    }

    // Row 8: Advice
    if (label.advice) {
        labelContent += `<div style="${buildRowStyle(rowStyles.advice)}">หมายเหตุ: ${escapeHtml(label.advice)}</div>`;
    }
