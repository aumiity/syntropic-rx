    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('btn-add-label')?.addEventListener('click', function () {
            ...
            newCard.innerHTML = `
                ...
                <div class="flex-1">
                    <div class="label-inline-form space-y-3" data-label-id="new">
                        ...
                        <div class="flex gap-2 pt-3 border-t border-gray-100 mt-2">
                            <button type="button" class="save-label-btn px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium">💾 บันทึก</button>
            ...
            // Bind save for new label (use click to avoid nested-form submit quirks)
            const saveNewBtn = newForm?.querySelector('.save-label-btn');
            ...
            // Keep Enter key from bubbling to the main form submit
        ...
        function initializeLabelAutocomplete(config = null) {
            ...
            searchInput.addEventListener('blur', () => {
                // Small delay so click on dropdown option fires before blur closes it
                setTimeout(() => {
                    dropdown.classList.add('hidden');
                }, 150);
            });
        ...
        function initializeAllLabelAutocompletes(container) {
        ...
        async function renderLabelsTable(labels) {
            ...
            for (const label of labels) {
                html += `
                    <div class="label-card bg-white border border-gray-200 rounded-xl shadow-sm p-4 mb-4 flex gap-4">
                        ...
                        <div class="flex-1">
                            <div class="label-inline-form space-y-3" data-label-id="${label.id}">
                                <div class="grid grid-cols-2 gap-4">
            ...
            // Bind save button for each inline form (silent save)
            container.querySelectorAll('.save-label-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    const form = btn.closest('.label-inline-form');
                    if (!form) return;
                    const labelId = form.dataset.labelId;
            ...
            initializeAllLabelAutocompletes(container);
        ...
        document.addEventListener('click', function(e) {
            ...
            // Close ALL visible label autocomplete dropdowns
            // unless the click was inside that dropdown or its paired search input / clear button
            document.querySelectorAll(
                '[id^="dosage_id_dropdown-"], [id^="frequency_id_dropdown-"], ' +
                '[id^="meal_relation_id_dropdown-"], [id^="label_time_id_dropdown-"], ' +
                '[id^="advice_id_dropdown-"]'
            ).forEach(dropdown => {
                if (dropdown.classList.contains('hidden')) return;
                // Extract the label suffix (e.g. "4", "5", "new")
                const match = dropdown.id.match(/-(\w+)$/);
                const suffix = match ? match[1] : null;
                if (!suffix) { dropdown.classList.add('hidden'); return; }

                const inDropdown = target.closest(`#${dropdown.id}`);
                const inSearch   = suffix ? target.closest(`[id$="_search-${suffix}"]`) : false;
                const inClear    = suffix ? target.closest(`[id$="-${suffix}"]`) : false;
                if (!inDropdown && !inSearch && !inClear) {
                    dropdown.classList.add('hidden');
                }
            });

            // Also close the old-style modal autocomplete dropdowns (no suffix)
